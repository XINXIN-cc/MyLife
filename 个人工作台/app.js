...
function lcServerURL(appId){return `https://${String(appId||'').trim().slice(0,8)}.api.lncldglobal.com`;}
...
$('cloudPushBtn')?.addEventListener('click',async()=>{if(!cloudSession?.user?.id){showToast('请先登录');return;}setCloudBusy(true);try{const payload=JSON.stringify(collectBackup());const where=encodeURIComponent(JSON.stringify({owner:cloudSession.user.id}));const exist=await cloudFetch(`/1.1/classes/WorkbenchData?where=${where}`,{},true);let objId=exist?.results?.[0]?.objectId;if(objId){await cloudFetch(`/1.1/classes/WorkbenchData/${objId}`,{method:'PUT',body:JSON.stringify({payload})},true);}else{await cloudFetch('/1.1/classes/WorkbenchData',{method:'POST',body:JSON.stringify({owner:cloudSession.user.id,payload})},true);}renderCloudState('已同步');showToast('当前记录已上传云端');}catch(e){showToast(`上传失败：${e.message}`);}finally{setCloudBusy(false);}});
