...
async function cloudAuth(mode){if(!readCloudConfig())return;var email=$('cloudEmail').value.trim(),pwd=$('cloudPassword').value;if(!email||pwd.length<6){showToast('请填写邮箱和至少6位密码');return;}setCloudBusy(true);if($('authCloudStatus'))$('authCloudStatus').textContent=mode==='signup'?'正在注册，请稍候...':'正在登录，请稍候...';try{var data=await yesapiFetch(mode==='signup'?'App.User.Register':'App.User.Login',{username:email,password:md5Hex(pwd),ext_info:JSON.stringify({email})},'POST');if(data.uuid&&data.token){cloudSession={uuid:data.uuid,token:data.token,username:email};save(CLOUD_SESSION_KEY,cloudSession);var switched=activateLocalProfile(data.uuid);renderCloudState();showToast(mode==='signup'?'🎉 注册并登录成功！':'✅ 登录成功！');if(switched)setTimeout(function(){location.reload();},500);}else{showToast(mode==='signup'?'注册完成':'登录异常');setAuthMode('login');}}catch(e){if($('authCloudStatus'))$('authCloudStatus').textContent='❌ '+e.message;showToast(e.message);}finally{setCloudBusy(false);}}
