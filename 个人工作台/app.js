...
function friendlyCloudError(error){const message=String(error?.message||error||'');if(error instanceof TypeError||/failed to fetch|networkerror|load failed/i.test(message))return new Error('无法连接云端项目，请检查 Supabase Project URL 是否完整正确');return error instanceof Error?error:new Error(message||'云端请求失败');}
...
async function cloudFetch(path,options={},auth=false,retried=false){if(!cloudConfig.url||!cloudConfig.anonKey)throw new Error('云端尚未配置');const ac=new AbortController();const timer=setTimeout(()=>ac.abort(),15000);try{const res=await fetch(cloudConfig.url+path,{...options,signal:ac.signal,headers:{...cloudHeaders(auth),...(options.headers||{})}});clearTimeout(timer);if(res.status===401&&auth&&!retried){await refreshCloudSession();return cloudFetch(path,options,auth,true);}let body=null;try{body=await res.json();}catch(e){}if(!res.ok)throw new Error(body?.msg||body?.message||body?.error_description||`请求失败 ${res.status}`);return body;}catch(e){clearTimeout(timer);if(e.name==='AbortError')throw new Error('连接云端超时，请检查 Supabase Project URL 是否正确');throw friendlyCloudError(e);}}
...
$('cloudForgotBtn')?.addEventListener('click',async()=>{if(!readCloudConfig())return;const email=$('cloudEmail').value.trim();if(!email){showToast('请先填写注册邮箱');return;}setCloudBusy(true);try{const redirect=location.href.split('#')[0];await cloudFetch(`/auth/v1/recover?redirect_to=${encodeURIComponent(redirect)}`,{method:'POST',body:JSON.stringify({email})});$('authCloudStatus').textContent='重置邮件已发送，请检查邮箱';showToast('重置密码邮件已发送');}catch(e){showToast(e.message);}finally{setCloudBusy(false);}});
let cloudRecoveryToken='';
function initPasswordRecovery(){const p=new URLSearchParams(location.hash.slice(1));if(p.get('type')!=='recovery'||!p.get('access_token'))return;cloudRecoveryToken=p.get('access_token');$('authGate').classList.remove('hidden');$('authTabs').classList.add('hidden');$('authResetPanel').classList.remove('hidden');$('authCloudStatus').textContent='请输入新的登录密码';}
$('cloudResetPasswordBtn')?.addEventListener('click',async()=>{const password=$('cloudNewPassword').value;if(password.length<6){showToast('新密码至少需要6位');return;}try{const res=await fetch(cloudConfig.url+'/auth/v1/user',{method:'PUT',headers:{apikey:cloudConfig.anonKey,Authorization:`Bearer ${cloudRecoveryToken}`,'Content-Type':'application/json'},body:JSON.stringify({password})}),data=await res.json();if(!res.ok)throw new Error(data.message||'密码更新失败');history.replaceState(null,'',location.pathname+location.search);cloudRecoveryToken='';$('authResetPanel').classList.add('hidden');$('authTabs').classList.remove('hidden');setAuthMode('login');showToast('密码已更新，请重新登录');}catch(e){showToast(e.message);}});
