...
/* ===== 可选云同步（LeanCloud 国际版 REST，无配置时保持纯本地模式） ===== */
const CLOUD_CONFIG_KEY='wb_cloud_config',CLOUD_SESSION_KEY='wb_cloud_session',CLOUD_PRESET=window.WORKBENCH_CLOUD_CONFIG||{};
function lcServerURL(appId){return `https://${String(appId||'').trim().toLowerCase().slice(0,8)}.api.lncldglobal.com`;}
let cloudConfig=load(CLOUD_CONFIG_KEY,{appId:'',appKey:''}),cloudSession=load(CLOUD_SESSION_KEY,null);
if(CLOUD_PRESET.appId&&CLOUD_PRESET.appKey)cloudConfig={appId:CLOUD_PRESET.appId.trim(),appKey:CLOUD_PRESET.appKey.trim(),serverURL:lcServerURL(CLOUD_PRESET.appId)};
...
function cloudHeaders(auth=false){const h={'X-LC-Id':cloudConfig.appId||'','X-LC-Key':cloudConfig.appKey||'','Content-Type':'application/json'};if(auth&&cloudSession?.sessionToken)h['X-LC-Session']=cloudSession.sessionToken;return h;}
function friendlyCloudError(error){const message=String(error?.message||error||'');if(error instanceof TypeError||/failed to fetch|networkerror|load failed|abort/i.test(message))return new Error('无法连接云端，请检查网络或 LeanCloud App ID 是否正确');return error instanceof Error?error:new Error(message||'云端请求失败');}
function setCloudBusy(busy){
  const labels = { cloudTestBtn:['测试连接','⏳ 测试中...'], cloudSignupBtn:['创建我的工作台','⏳ 注册中...'], cloudLoginBtn:['登录','⏳ 登录中...'], cloudPushBtn:['上传备份','⏳ 上传中...'], cloudPullBtn:['下载备份','⏳ 下载中...'] };
  Object.entries(labels).forEach(([id,[normal,loading]])=>{
    const el=$(id); if(!el) return;
    el.disabled=busy; el.textContent=busy?loading:normal;
    if(busy){el.dataset.origClass=el.className;el.classList.add('btn-loading');}
    else{el.className=el.dataset.origClass||'';el.classList.remove('btn-loading');}
  });
  /* 在认证区域显示/隐藏 loading 提示 */
  const tip=$('authLoadingTip');
  if(tip) tip.style.display=busy?'':'none';
}
function renderCloudState(msg=''){
  if(!$('cloudStatus'))return;$('cloudAppId').value=cloudConfig.appId||'';$('cloudAppKey').value=cloudConfig.appKey||'';const configured=!!(cloudConfig.appId&&cloudConfig.appKey&&cloudConfig.serverURL),logged=!!cloudSession?.sessionToken;$('authGate')?.classList.toggle('hidden',logged);$('authProjectConfig')?.classList.toggle('hidden',configured);$('cloudUserPanel').classList.toggle('hidden',!logged);$('cloudUserText').textContent=logged?`已登录：${cloudSession.user?.email||'当前用户'}`:'';const status=msg||(logged?'已登录':configured?'云端已连接':'等待管理员配置');$('cloudStatus').textContent=status;$('cloudStatus').classList.toggle('online',logged);if($('authCloudStatus'))$('authCloudStatus').textContent=status;
}
function readCloudConfig(){const appId=$('cloudAppId').value.trim(),appKey=$('cloudAppKey').value.trim();if(!appId||!appKey){showToast('请先填写 LeanCloud App ID 和 App Key');return false;}cloudConfig={appId,appKey,serverURL:lcServerURL(appId)};save(CLOUD_CONFIG_KEY,cloudConfig);return true;}
async function cloudFetch(path,options={},auth=false){if(!cloudConfig.appId||!cloudConfig.appKey||!cloudConfig.serverURL)throw new Error('云端尚未配置');const ac=new AbortController();const timer=setTimeout(()=>ac.abort(),15000);try{const res=await fetch(cloudConfig.serverURL+path,{...options,signal:ac.signal,headers:{...cloudHeaders(auth),...(options.headers||{})}});clearTimeout(timer);let body=null;try{body=await res.json();}catch(e){}if(res.status===401)throw new Error('登录已过期，请重新登录');if(!res.ok)throw new Error(body?.error||body?.message||`请求失败 ${res.status}`);return body;}catch(e){clearTimeout(timer);if(e.name==='AbortError')throw new Error('连接云端超时，请检查网络或 LeanCloud App ID 是否正确');throw friendlyCloudError(e);}}
$('cloudTestBtn')?.addEventListener('click',async()=>{if(!readCloudConfig())return;setCloudBusy(true);try{await cloudFetch('/1.1/date');renderCloudState('连接正常，可以注册或登录');showToast('云端项目连接成功');}catch(e){renderCloudState('连接失败：请检查 App ID 和 App Key');showToast(e.message);}finally{setCloudBusy(false);}});
async function cloudAuth(mode){if(!readCloudConfig())return;const email=$('cloudEmail').value.trim(),password=$('cloudPassword').value;if(!email||password.length<6){showToast('请填写邮箱和至少6位密码');return;}setCloudBusy(true);if($('authCloudStatus'))$('authCloudStatus').textContent=mode==='signup'?'正在注册，请稍候...':'正在登录，请稍候...';try{const path=mode==='signup'?'/1.1/users':'/1.1/login';const data=await cloudFetch(path,{method:'POST',body:JSON.stringify(mode==='signup'?{username:email,email,password}:{username:email,password})});if(data.sessionToken){cloudSession={sessionToken:data.sessionToken,user:{id:data.objectId,email:data.email||email}};save(CLOUD_SESSION_KEY,cloudSession);const switched=activateLocalProfile(data.objectId);renderCloudState();showToast(mode==='signup'?'🎉 注册并登录成功！':'✅ 登录成功！');if(switched)setTimeout(()=>location.reload(),500);}else{showToast('注册成功，请前往邮箱确认后再登录');setAuthMode('login');}}catch(e){if($('authCloudStatus'))$('authCloudStatus').textContent='❌ '+e.message;showToast(e.message);}finally{setCloudBusy(false);}}
...
$('cloudForgotBtn')?.addEventListener('click',async()=>{if(!readCloudConfig())return;const email=$('cloudEmail').value.trim();if(!email){showToast('请先填写注册邮箱');return;}setCloudBusy(true);try{await cloudFetch('/1.1/requestPasswordReset',{method:'POST',body:JSON.stringify({email})});$('authCloudStatus').textContent='重置邮件已发送，请检查邮箱（若未收到，可能需在 LeanCloud 控制台配置邮件服务）';showToast('重置密码邮件已发送');}catch(e){showToast(e.message);}finally{setCloudBusy(false);}});
// 密码重置走 LeanCloud 邮件链接（链接指向应用设置的回调页），前端仅触发邮件发送
$('cloudResetPasswordBtn')?.addEventListener('click',async()=>{showToast('请前往邮箱点击重置链接完成改密');});
$('cloudPushBtn')?.addEventListener('click',async()=>{if(!cloudSession?.user?.id){showToast('请先登录');return;}setCloudBusy(true);try{const payload=JSON.stringify(collectBackup());const uid=cloudSession.user.id;const acl={[uid]:{read:true,write:true}};const where=encodeURIComponent(JSON.stringify({owner:uid}));const exist=await cloudFetch(`/1.1/classes/WorkbenchData?where=${where}`,{},true);let objId=exist?.results?.[0]?.objectId;if(objId){await cloudFetch(`/1.1/classes/WorkbenchData/${objId}`,{method:'PUT',body:JSON.stringify({payload,ACL:acl})},true);}else{await cloudFetch('/1.1/classes/WorkbenchData',{method:'POST',body:JSON.stringify({owner:uid,payload,ACL:acl})},true);}renderCloudState('已同步');showToast('当前记录已上传云端');}catch(e){showToast(`上传失败：${e.message}`);}finally{setCloudBusy(false);}});
$('cloudPullBtn')?.addEventListener('click',async()=>{if(!cloudSession?.user?.id){showToast('请先登录');return;}if(!confirm('下载云端数据会覆盖当前账号的文字记录，是否继续？'))return;setCloudBusy(true);try{const where=encodeURIComponent(JSON.stringify({owner:cloudSession.user.id}));const rows=await cloudFetch(`/1.1/classes/WorkbenchData?where=${where}`,{},true);if(!rows?.results?.length){showToast('云端还没有备份');return;}downloadBackup('云端恢复前自动备份');let backup;try{backup=JSON.parse(rows.results[0].payload);}catch(e){throw new Error('云端数据格式不正确');}if(backup?.app!=='personal-workbench'||!backup.data)throw new Error('云端数据格式不正确');restoreLocalSnapshot(backup.data);save('wb_profile_cache_'+cloudSession.user.id,backup.data);showToast('云端数据下载成功');setTimeout(()=>location.reload(),700);}catch(e){showToast(`下载失败：${e.message}`);}finally{setCloudBusy(false);}});
$('cloudLogoutBtn')?.addEventListener('click',async()=>{try{await cloudFetch('/1.1/logout',{method:'POST'},true);}catch(e){}const current=localStorage.getItem('wb_active_profile')||cloudSession?.user?.id;if(current&&current!=='guest')save('wb_profile_cache_'+current,currentLocalSnapshot());restoreLocalSnapshot(load('wb_profile_cache_guest',{}));localStorage.setItem('wb_active_profile','guest');cloudSession=null;localStorage.removeItem(CLOUD_SESSION_KEY);renderCloudState('已退出');setTimeout(()=>location.reload(),400);});
renderCloudState();
