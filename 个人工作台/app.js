...
/* ===== 可选云同步（Cloudflare Pages Functions + KV，同源免域名、无需任何外部配置） ===== */
const CLOUD_CONFIG_KEY='wb_cloud_config',CLOUD_SESSION_KEY='wb_cloud_session';
let cloudConfig={enabled:true},cloudSession=load(CLOUD_SESSION_KEY,null);
...
async function apiFetch(path,body){var ac=new AbortController(),timer=setTimeout(function(){ac.abort();},15000);try{var res=await fetch(path,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body),signal:ac.signal});clearTimeout(timer);var j=await res.json().catch(function(){return {};});if(j.ret!==200)throw new Error(j.msg||('请求失败('+j.ret+')'));return j;}catch(err){clearTimeout(timer);if(err.name==='AbortError')throw new Error('连接云端超时，请稍候重试');throw friendlyCloudError(err);}}
function setCloudBusy(busy){
  var L={cloudSignupBtn:['创建我的工作台','⏳ 注册中...'],cloudLoginBtn:['登录并进入','⏳ 登录中...'],cloudPushBtn:['↑ 上传当前数据','⏳ 上传中...'],cloudPullBtn:['↓ 下载云端数据','⏳ 下载中...']};
...
function renderCloudState(msg){
  if(!$('cloudStatus'))return;var logged=!!cloudSession?.token;$('authGate')?.classList.toggle('hidden',logged);$('authProjectConfig')?.classList.add('hidden');$('cloudUserPanel')?.classList.toggle('hidden',!logged);$('cloudUserText').textContent=logged?'已登录：'+(cloudSession.username||'当前用户'):'';var st=msg||(logged?'已登录云端':'未登录云端');$('cloudStatus').textContent=st;$('cloudStatus').classList.toggle('online',logged);if($('authCloudStatus'))$('authCloudStatus').textContent=st;
}
/* 云同步已改为 Cloudflare 同源 /api 接口，无需任何外部配置 */

async function cloudAuth(mode){var email=$('cloudEmail').value.trim().toLowerCase(),pwd=$('cloudPassword').value;if(!email||pwd.length<6){showToast('请填写邮箱和至少6位密码');return;}setCloudBusy(true);if($('authCloudStatus'))$('authCloudStatus').textContent=mode==='signup'?'正在注册，请稍候...':'正在登录，请稍候...';try{var data=await apiFetch(mode==='signup'?'/api/register':'/api/login',{email:email,password:pwd});if(data&&data.token){cloudSession={token:data.token,email:data.email,username:data.email};save(CLOUD_SESSION_KEY,cloudSession);var switched=activateLocalProfile(data.email);renderCloudState();showToast(mode==='signup'?'🎉 注册并登录成功！':'✅ 登录成功！');if(switched)setTimeout(function(){location.reload();},500);}else{showToast('登录异常：未获取到会话凭证');setAuthMode('login');}}catch(e){if($('authCloudStatus'))$('authCloudStatus').textContent='❌ '+e.message;showToast(e.message);}finally{setCloudBusy(false);}}
...
$('cloudForgotBtn')?.addEventListener('click',async function(){var email=$('cloudEmail').value.trim();if(!email){showToast('请先填写注册邮箱');return;}setCloudBusy(true);try{showToast('请联系管理员重置密码，或使用邮箱作为用户名重新注册');}catch(e){showToast(e.message);}finally{setCloudBusy(false);}});
$('cloudResetPasswordBtn')?.addEventListener('click',async function(){showToast('请联系管理员重置密码');});
$('cloudPushBtn')?.addEventListener('click',async function(){if(!cloudSession?.token){showToast('请先登录');return;}setCloudBusy(true);try{await apiFetch('/api/upload',{token:cloudSession.token,payload:JSON.stringify(collectBackup())});renderCloudState('已同步');showToast('当前记录已上传云端');}catch(e){showToast('上传失败：'+e.message);}finally{setCloudBusy(false);}});
$('cloudPullBtn')?.addEventListener('click',async function(){if(!cloudSession?.token){showToast('请先登录');return;}if(!confirm('下载云端数据会覆盖当前账号的文字记录，是否继续？'))return;setCloudBusy(true);try{var res=await apiFetch('/api/download',{token:cloudSession.token});downloadBackup('云端恢复前自动备份');var backup;try{backup=JSON.parse(res.payload);}catch(er){throw new Error('云端数据格式不正确');}if(backup?.app!=='personal-workbench'||!backup.data)throw new Error('云端数据格式不正确');restoreLocalSnapshot(backup.data);save('wb_profile_cache_'+cloudSession.email,backup.data);showToast('云端数据下载成功');setTimeout(function(){location.reload();},700);}catch(e){showToast('下载失败：'+e.message);}finally{setCloudBusy(false);}});
$('cloudLogoutBtn')?.addEventListener('click',async function(){var cur=localStorage.getItem('wb_active_profile')||cloudSession?.email;if(cur&&cur!=='guest')save('wb_profile_cache_'+cur,currentLocalSnapshot());restoreLocalSnapshot(load('wb_profile_cache_guest',{}));localStorage.setItem('wb_active_profile','guest');cloudSession=null;localStorage.removeItem(CLOUD_SESSION_KEY);renderCloudState('已退出');setTimeout(function(){location.reload();},400);});
renderCloudState();
